	...
	return func(w http.ResponseWriter, r *http.Request) {
		defer func() {
			if err := recover(); err != nil {
				...
				fmt.Fprintf(w, "<h1>Some thing went wrong panic: %v</h1><pre>%s</pre>", err, makeLinks(string(stack)))
			}
		}()
		app.ServeHTTP(w, r)
	}
...
func panicAfterDemo(w http.ResponseWriter, r *http.Request) {
	fmt.Fprint(w, "<h1>Hello..! </h1></br><h2> panicAfterDeme..</h2>")
	funcThatPanics()
}

func funcThatPanics() {
	panic("Oh no..! Some thing went wrong..Please have look Source code..")
}

func hello(w http.ResponseWriter, r *http.Request) {
	fmt.Fprintln(w, "<h1>Hello...!</h1><h2>We have link when we got Error and Which File line Number as well</h2>")
}
